...
module.exports = function createFinanceiroCoreRoutes(deps) {
    ...
    router.get('/categorias', authenticateToken, checkFinanceiroPermission('visualizar'), async (req, res) => {
        try {
            ...
            let query = 'SELECT id, nome, tipo, cor, icone, orcamento_mensal, descricao, pai_id, ativo, created_at FROM categorias_financeiras WHERE 1=1';
            const params = [];
            ...
            query += ' ORDER BY COALESCE(pai_id, id), CASE WHEN pai_id IS NULL THEN 0 ELSE 1 END, nome ASC';
