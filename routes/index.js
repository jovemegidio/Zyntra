...
module.exports = function registerAllRoutes(app, deps) {
    ...
    try {
        ...
        // Estrutura de Produtos (BOM) — montado ANTES de /api/pcp p/ a rota específica vencer.
        try {
            const createPcpEstruturaRoutes = require('./pcp-estrutura-routes');
            app.use('/api/pcp/estrutura', createPcpEstruturaRoutes(sharedDeps));
            console.log('[ROUTES] ✅ PCP Estrutura montado em /api/pcp/estrutura');
        } catch (eEst) { console.error('[ROUTES] pcp-estrutura mount err:', eEst.message); }
        app.use('/api/pcp', pcpRouter);

        // Bridge: /api/gerar-ordem-pdf → pcpRouter (geração de ordem PDF via FOP) — paralelo ao Excel.
        // O frontend (modal_nova_ordem_saas.html, ordens-producao.html) chama esse caminho direto;
        // sem essa ponte a rota só existia em /api/pcp/api/gerar-ordem-pdf e dava 404.
        app.post('/api/gerar-ordem-pdf', authenticateToken, (req, res, next) => {
            req.url = '/gerar-ordem-pdf';
            pcpRouter(req, res, next);
        });
        console.log('[ROUTES] ✅ Bridge: /api/gerar-ordem-pdf → pcpRouter');
    ...
    try {
        // Modais de Vendas (notas-complementares, devolucoes, remessas, rastreabilidade) —
        // arquivo existia desde 18/jun/2026 mas nunca foi montado, causando 404 em produção
        // em notas-complementares.html/devolucoes.html/remessas.html/rastreabilidade.html.
        const createVendasModais = require('./vendas-modais-routes');
        app.use('/api/vendas-modais', createVendasModais(sharedDeps));
        console.log('[ROUTES] ✅ Vendas modais mounted at /api/vendas-modais');
    } catch (err) {
        console.error('[ROUTES] ❌ Failed to load vendas-modais-routes:', err.message);
    }
    ...
    const externalApis = [
        ...
        { path: '/api/relatorios-gerenciais', file: '../api/relatorios-gerenciais', name: 'Relatórios Gerenciais' },
